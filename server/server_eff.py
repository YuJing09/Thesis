...
import random
from obdatagen import trainimg_path,trainlabel_path,valimg_path
import six
if six.PY3:
  buffer_ = memoryview
else:
...
if __name__ == "__main__":
  ...
  train_path =trainimg_path
    

  #  for validation
  validation_path =valimg_path
  if args.validation:
    datapath = validation_path
  else:
    datapath = train_path
  
  gen = datagen(datapath,time_len=args.time, batch_size=args.batch)
  if args.validation:
    start_server(gen,5556,hwm=args.buffer)
  else:
