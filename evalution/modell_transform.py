...
class Ypgru(tf.keras.Model):
  ...
  def call(self,input1,input2):
        ...
        output=tf.keras.layers.concatenate([output1,output3,output2,output],axis=-1)
        return output
class gru(tf.keras.Model):
  def __init__(self,compound_coef,global_params,train=True):
    super().__init__()
    cls='efficientnet-b'+str(compound_coef)
    self._block_args,self._global_params=get_model_params(cls,None)
    self.backbone_net=EfficientNet(self._block_args,self._global_params,extract_features=True)
  ...
  def call(self,input1,input2):
        ...
        x = self._relu_fn(self._bn2(self._conv_extract(input1)))
        #print(x.shape)
...
class Ypgru2(tf.keras.Model):
  ...
  def call(self,input1,input2):
        ...
        r=self.sigmoid(r)
        z=self.sigmoid(z)
        rnn_rh=self.multiply([h,r])
        rnn_rh=self.add([rnn_rh,rnn_h])
        rnn_rh=self.tanh(rnn_rh)
        
        ht=self.multiply([h,1-z])
        h=self.multiply([rnn_rh,z])	
        #state
        output=self.add([h,ht])
        #path
        output1=self.path_dense(output)
