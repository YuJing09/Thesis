  ...
  def call(self,inputs):
    ...
    if not self._extract_features:
     ...
     # x = self._fc(x)
      ...
      return x
class Ypregsteer(tf.keras.Model):
  ...
  def _build(self):
    ...
    self.steer_dense=tf.keras.layers.Dense(256,kernel_initializer='normal',name='steer_dense')
    self.steer_dense2=tf.keras.layers.Dense(256,kernel_initializer='normal',name='steer_dense2')
    self.steer_dense3=tf.keras.layers.Dense(256,kernel_initializer='normal',name='steer_dense3')
    self.steer_dense4=tf.keras.layers.Dense(128,kernel_initializer='normal',name='steer_dense4')
    self.steer_dense5=tf.keras.layers.Dense(1,kernel_initializer='normal',name='steer_dense51') # 50 points 1 vaild_len
  ...
  def call(self,input1):
    ...
    output = tf.keras.layers.Flatten()(x)
     #steer
    output1=self.steer_dense(output)
    output1=self._relu(output1)
    output1=self.dropout0(output1)
    output1=self.steer_dense2(output1)
    output1=self._relu(output1)
    output1=self.dropout0(output1)
    output1=self.steer_dense3(output1)
    output1=self._relu(output1)
    output1=self.dropout0(output1)
    output1=self.steer_dense4(output1)
    output1=self._relu(output1)
    output1=self.dropout1(output1)
    output1=self.steer_dense5(output1)
#lead
    ...
    return output1
class Ypgru_steer(tf.keras.Model):
  def __init__(self,compound_coef,train=True):
    super().__init__()
    cls='efficientnet-b'+str(compound_coef)
    self._block_args,self._global_params=get_model_params(cls,None)
  ...
  def _build(self):
    ...
    self.steer_dense3=tf.keras.layers.Dense(256,kernel_initializer='normal',name='steer_dense3')
    self.steer_dense4=tf.keras.layers.Dense(128,kernel_initializer='normal',name='steer_dense4')
    self.steer_dense5=tf.keras.layers.Dense(1,kernel_initializer='normal') # steer,steer_std
  ...
  def call(self,input1,input2):
        ...
        output=tf.keras.layers.concatenate([output1,output],axis=-1)
        return output
