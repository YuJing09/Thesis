...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
      ...
      if self._block_args.expand_ratio != 1:
            x = tf.keras.layers.TimeDistributed(self._expand_conv)(inputs)
            x = self._bn0(x)
            x = self._relu_fn(x)
            

      x = tf.keras.layers.TimeDistributed(self._depthwise_conv)(x)
      x = self._bn1(x)
      x = self._relu_fn(x)
      ...
      x = tf.keras.layers.TimeDistributed(self._project_conv)(x)
      x = self._bn2(x)
...
class EfficientNet(tf.keras.Model):
  ...
  def call(self,inputs):
    ...
    inputs=tf.keras.layers.Lambda(lambda x : x/255.,input_shape=(inputs.shape[1],inputs.shape[2],inputs.shape[3],inputs.shape[4]))(inputs)
...
class PAM_Module(tf.keras.layers.Layer):
  ...
  def call(self, x):
        ...
        proj_query = tf.reshape(proj_query,[m_batchsize,-1,width*height])
   
        proj_query=tf.transpose(proj_query,perm=[0,2,1])
        
        # 矩阵C
        proj_key = tf.reshape(tf.transpose(self.key_conv(x),perm=[0,3,1,2]),[m_batchsize,-1,width*height])
        
        # torch.bmm点积操作
        energy = tf.matmul(proj_query, proj_key)
        # 映射到0-1区间的系数
        attention = self.softmax(energy)
        proj_value = tf.reshape(tf.transpose(self.value_conv(x),perm=[0,3,1,2]),[m_batchsize,-1,width*height])
...
class CAM_Module(tf.keras.layers.Layer):
    ...
    def call(self,x):
        ...
        out = tf.transpose(out,perm=[0,2,3,1])
        return out
class TAM_Module(tf.keras.layers.Layer):
    def __init__(self,
                 in_channels,
                 n_segment,
                 kernel_size=3,
        ...
        self.G.add(tf.keras.layers.Dense(n_segment * 2,use_bias=False))
        self.G.add(tf.keras.layers.BatchNormalization(axis=-1))
        self.G.add(tf.keras.layers.ReLU())
        self.G.add(tf.keras.layers.Dense(kernel_size,use_bias=False))
        self.G.add(tf.keras.layers.Softmax())
            

        self.L = tf.keras.Sequential()
        self.L.add(tf.keras.layers.Conv1D(in_channels//4,kernel_size,strides=1,padding='same',use_bias=False))
        self.L.add(tf.keras.layers.BatchNormalization(axis=-1))
        self.L.add(tf.keras.layers.ReLU())    
        self.L.add(tf.keras.layers.Conv1D(in_channels,1,padding='valid',use_bias=False))
        self.L.add( tf.keras.layers.Activation('sigmoid'))
    ...
    def call(self, x):
        ...
        new_x =tf.transpose(x,perm=[0,4,2,3,1]) # b,c,h,w,t
        out = tf.keras.layers.AveragePooling2D(pool_size=(h, w))(tf.reshape(new_x,[batch*c,h,w,t]))
        
        out = tf.reshape(out,[-1, t])
    
        conv_kernel = self.G(out)
        conv_kernel = tf.reshape(conv_kernel,[batch*c,1,-1,1])
        conv_kernel = tf.transpose(conv_kernel,perm=[2,3,0,1])
        out=tf.reshape(out,[batch,c,t])
        out=tf.transpose(out,perm=[0,2,1])
        local_activation = self.L(out)
        local_activation=tf.transpose(local_activation,perm=[0,2,1])
        local_activation = tf.reshape(local_activation,[batch,c,t,1,1])
        new_x =tf.transpose(new_x,perm=[0,1,4,2,3]) # b,c,t,h,w
        new_x = new_x * local_activation
        new_x=tf.reshape(new_x,[1, batch * c, t, h * w])
        new_x = tf.transpose(new_x,perm=[0,2,3,1]) # 1 ,(b*c),t,(h*w)
        out = tf.nn.depthwise_conv2d(new_x,
                       conv_kernel,
                       strides=(1,1,self.stride, 1),
                       padding= [[0, 0], [1,1], [0,0], [0, 0]])
        out=tf.transpose(out,perm=[3,1,2,0])
        out=tf.reshape(out,[batch,c,t,h,w])
        out=tf.transpose(out,perm=[0,2,1,3,4])
        out=tf.reshape(out,[batch*t,c,h,w])
        return out
...
class Ypdanet(tf.keras.Model):
  ...
  def _build(self):
    ...
    self.lead_dense5=tf.keras.layers.Dense(5,kernel_initializer='normal',name='lead_dense51') # 
...
if __name__ == '__main__':
  ...
  input1=tf.keras.Input(shape=(5,10,1408),name='img')
  ...
  da=DANet(512,32)
  outda=da.call(input1)
  model=tf.keras.Model(inputs=[input1],outputs=outda)
  model.summary()
  #TAM=TAM_Module(128,10)
  #x=tf.random.uniform((2,10,5,10,128),minval=0,maxval=1,dtype=tf.float32)
  #TAM.call(x)
  #model=Ypdanet(2)
  #output=model.call(input1)
  #model= tf.keras.Model(inputs=[input1],outputs=output)
