...
def warp_points(pt_s, warp_matrix):
  ...
  #print(warp_matrix.shape,pt_s)
  #print(warp_matrix.shape,pt_s.shape)
  pt_d = np.dot(warp_matrix[:, :-1], pt_s.T)
  #print(pt_d.shape)
...
def draw_path(y, x,z, color, img, calibration, top_down, lid_color=None,line=True,stds=None,prob=1):
  # TODO: Remove big box.
  
  uv_model_real = warp_points(np.column_stack((x, y,z)), calibration.car_to_bb())
  #print(uv_model_real)
  ...
  uv_model_dots = uv_model[filter_index]
  
  if type(stds)!= type(None):
    device_path_l = np.column_stack((x, y,z))                                                                  
    device_path_r = np.column_stack((x, y,z))
    device_path_l[:,1] -= stds                                                                                            
    device_path_r[:,1] += stds
    uv_model_l=warp_points(device_path_l, calibration.car_to_bb())
    uv_model_r=warp_points(device_path_r, calibration.car_to_bb())
    uv_model_l = np.round(uv_model_l).astype(int)
    uv_model_r = np.round(uv_model_r).astype(int)
    uv_model_dots_l=uv_model_l[filter_index]
    uv_model_dots_r=uv_model_r[filter_index]
    for i in range(1, len(uv_model_dots_l)):
      u1,v1,u2,v2 = np.append(uv_model_dots_l[i-1], uv_model_dots_r[i-1])
      u3,v3,u4,v4 = np.append(uv_model_dots_l[i], uv_model_dots_r[i])
      pts = np.array([[u1,v1],[u2,v2],[u4,v4],[u3,v3]], np.int32).reshape((-1,1,2))
      #cv2.fillPoly(img,[pts],(174,48,96))
      cv2.polylines(img,[pts],True,(174,48,255))
    #print(uv_model_dots_l)
  
  a=np.array(uv_model_dots,np.int32).reshape((-1,1,2))
  if line :                                                 
    cv2.polylines(img,[a],True,color*prob)
  #print(uv_model_dots.shape,y.shape)
  
  for i, j  in ((-1, 0), (0, -1), (0, 0), (0, 1), (1, 0)):
    
    
    img[uv_model_dots[:, 1] + i, uv_model_dots[:, 0] + j] = color*prob
