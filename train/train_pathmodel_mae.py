...
def custom_loss(y_true, y_pred,delta=2.):
  ...
  true_valid_len=tf.clip_by_value(y_true[:,50],0.,1000.)
  ...
  for i in range(batch_size):
    ...
    l=tf.cast(true_valid_len[i],dtype=tf.int32)
    ...
    lead_std_all_loss=tf.where(tf.equal(true_lead_prob[i],1),tf.reduce_sum(lead_std_loss),0) 
    #bce
    true_prob=true_lead_prob[i]
    pred_prob=pred_lead_prob[i]
    bce=-1.*(true_prob*tf.math.log(pred_prob)+(5.*(1.-true_prob)*tf.math.log(1.-pred_prob)))
    #bce=-1.*(true_prob*tf.math.log(pred_prob)+((1.-true_prob)*tf.math.log(1.-pred_prob)))
    #s add all loss
    all_loss=points_all_loss+points_std_all_loss+lead_all_loss+lead_std_all_loss+valid_len_loss+bce
  ...
  loss=s/tf.cast(batch_size,dtype=tf.float32)
...
if __name__=="__main__":
  ...
  #model.load_weights('./saved_model/weights-improvement-08-28.33.hdf5')
 
  history=model.fit_generator(
    gen(20, args.host, port=args.port,m=model),
    steps_per_epoch=20*1200/20.,
    epochs=args.epoch,
    validation_data=gen(20, args.host, port=args.val_port,m=model),
    validation_steps=20*1200/20.,verbose=1,callbacks=callbacks_list)
