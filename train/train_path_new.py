...
def custom_loss(y_true, y_pred,delta=2.):
  ...
  pred_valid_len=y_pred[:,100:101]
  true_std=tf.abs(y_true[:,:50]-y_pred[:,:50])
  pred_std=tf.math.exp(y_pred[:,50:100])
  ...
  pred_lead_prob=tf.clip_by_value(tf.math.sigmoid(y_pred[:,109]),1e-15,1-1e-15)
  #lead_prob_mse=tf.square(pred_lead_prob-true_lead_prob)/2.
  #print(true_lead_prob,pred_lead_prob)  
  li=tf.cast(tf.clip_by_value(y_true[:,50]*10.,0.,50.),dtype=tf.int32)
  ...
  error_valid_len=tf.abs(tf.subtract(true_valid_len[:,-1],pred_valid_len[:,-1]))
  ...
  condition_study_std=tf.less(error_point,0.5)  
  condition_study_zero_std=tf.less(error_zero_point,0.1)
  ...
  point_zero_point_std_loss=tf.where(condition_study_zero_std,point_zero_point_std_loss,0)
  std_loss=tf.where(condition_study_std,std_loss,0)
 ...
 # std_loss=tf.reduce_sum(std_loss[:l])
  ...
  #points loss
  points_all_loss=tf.where(tf.equal(li,0),point_zero_point_loss,(points_loss))
  
  points_std_all_loss=tf.where(tf.equal(li,0),point_zero_point_std_loss,(stds_loss))
  #lead huber loss
  ...
  condition_study_lead_std=tf.less(error_lead,1.)   
  ...
  lead_std_loss=tf.where(condition_study_lead_std,lead_std_loss,0.)
  #lead loss
  ...
  bce=-1.*(true_prob*tf.math.log(pred_prob)+(2.*(1.-true_prob)*tf.math.log(1.-pred_prob)))
  ...
  loss=points_all_loss+points_std_all_loss/15.+lead_all_loss+lead_std_all_loss/10.+valid_len_loss/2.+bce
...
if __name__=="__main__":
  ...
  model.load_weights('./saved_model/weights-improvement-27-3.46.hdf5')
  ...
  history=model.fit_generator(
    ...
    validation_steps=10*1200/25.,verbose=1,callbacks=callbacks_list)
  print("Saving model weights and configuration file.")

  
  
  model.save_weights("./saved_model/ypgru.keras", True)
  with open('./saved_model/ypgru.json', 'w') as outfile:
